...
var INCREMENTENEMIESFORLEVEL = 5;
var MAXENEMIESFORLEVEL = 25;
var PLAYERLIVES = 4;
var MAXPLAYERLIVES = 6;
var TIMEBETWEENLEVEL = 80;
var TIMEBEFOREINSTRUCTIONS = 20;
...
aNextLevel.src = pathMedia + "media/audio/powerup.mp3";
...
iBackground1.src = pathMedia + 'media/images/nebula1.jpg';
...
var aBackground = [iBackground1, iBackground2, iBackground3, iBackground4, iBackground5,
                   iBackground6, iBackground7, iBackground8, iBackground9];
...
iSpacecraft.src = pathMedia + 'media/images/nave.png';
...
iEnemy1.src = pathMedia + 'media/images/enemigo1.png';

var iEnemy1d = new Image();
iEnemy1d.src = pathMedia + 'media/images/enemigo1d.png';

var iEnemy2 = new Image();
iEnemy2.src = pathMedia + 'media/images/enemigo2.png';

var iEnemy2d = new Image();
iEnemy2d.src = pathMedia + 'media/images/enemigo2d.png';
            ...
            if (lastKey == 32) {
                ...
                if (timerShot == 0) {
                    ...
                    if (multishot == 1) {
                        ...
                        shots.push(new Rectangle(player.x + 30, player.y, 4, 16, "laser1"));
                    }

                    if (multishot == 2) {

                        shots.push(new Rectangle(player.x + 28, player.y, 4, 16, "laser2"));
                        shots.push(new Rectangle(player.x + 34, player.y, 4, 16, "laser2"));
                    }

                    if (multishot == 3) {

                        shots.push(new Rectangle(player.x + 18, player.y, 4, 16, "laser2"));
                        shots.push(new Rectangle(player.x + 28, player.y, 4, 16, "laser1"));
                        shots.push(new Rectangle(player.x + 34, player.y, 4, 16, "laser1"));
                        shots.push(new Rectangle(player.x + 44, player.y, 4, 16, "laser2"));
                    }

                    if (multishot == 4) {

                        shots.push(new Rectangle(player.x + 8, player.y, 4, 16, "laser3"));
                        shots.push(new Rectangle(player.x + 18, player.y, 4, 16, "laser2"));
                        shots.push(new Rectangle(player.x + 28, player.y, 4, 16, "laser1"));
                        shots.push(new Rectangle(player.x + 34, player.y, 4, 16, "laser1"));
                        shots.push(new Rectangle(player.x + 44, player.y, 4, 16, "laser2"));
                        shots.push(new Rectangle(player.x + 54, player.y, 4, 16, "laser3"));
                    }
                ...
                if (player.intersects(powerups[i])) {
                    ...
                    if (powerups[i].powerups == 1) {
                        ...
                        if (multishot < 4) {
                            ...
                            var myGun = ["GUN", "SHOTGUN", "MACHINE GUN", "ROCKET LAUNCHER"];
...
function paint(ctx) {
    ...
    if (instructions == 1) {
        ...
        ctx.drawImage(iInstructionsBackground, 0, 0, 900, 450);
        ctx.drawImage(iPanchoEspacial, 720, 90, 120, 300);
        ctx.drawImage(iGlobe, 630, 40, 120, 120);

        ctx.drawImage(iHologram, 200, 85, 390, 300);

        ctx.textAlign = 'center';

        ctx.fillStyle = '#000';
        ctx.font = 'bold 15pt Arial';
        ctx.fillText('Presiona', 690, 90);

        if (touchable)
            ctx.fillText('la pantalla', 690, 110);
        else
            ctx.fillText('enter', 694, 110);

        ctx.fillStyle = '#fff600';
        ctx.font = 'bold 19pt Arial';
        ctx.fillText('Instrucciones', (canvas.width / 2) -60, (canvas.height / 2) + 5);

        ctx.fillStyle = '#fff';
        ctx.font = 'bold 15pt Arial';
        ctx.fillText('Utiliza las teclas de flecha', (canvas.width / 2) - 57, (canvas.height / 2) + 45);
        ctx.fillText('para controlar la nave, pulsa la', (canvas.width / 2) - 57, (canvas.height / 2) + 70);
        ctx.fillText('barra espaciadora para disparar. ', (canvas.width / 2) - 57, (canvas.height / 2) + 95);
    ...
    ctx.drawImage(aBackground[indexBackground], 0, 450 + bgTimer);

    //Dibuja las bolas de fuego
    ctx.drawImage(iFireBall, fireBall.x, fireBall.y, fireBall.width, fireBall.height);
    ctx.drawImage(iFireBall2, fireBall2.x, fireBall2.y, fireBall2.width, fireBall2.height);
    ctx.drawImage(iFireBall3, fireBall3.x, fireBall3.y, fireBall3.width, fireBall3.height);

    //Dibuja el planeta
    ctx.drawImage(iPlanet2, planet2.x, planet2.y, planet2.width, planet2.height);
    ctx.drawImage(iPlanet4, planet4.x, planet4.y, planet4.width, planet4.height);
    ctx.drawImage(iPlanet3, planet3.x, planet3.y, planet3.width, planet3.height);
    ctx.drawImage(iPlanet, planet.x, planet.y, planet.width, planet.height);
    ctx.drawImage(iPlanet5, planet5.x, planet5.y, planet5.width, planet5.height);

    //Dibuja el asteroide
    ctx.drawImage(iAsteroid, asteroid.x, asteroid.y, asteroid.width, asteroid.height);
    ...
    ctx.drawImage(iSpaceCenter, 705, 324, 180, 108);
    ...
    ctx.drawImage(iPantera, 760, 6, 32, 32);
    ctx.fillText(score, 815 + ((score.toString().length - 1) * 10), 30);

    //Dibuja el nivel del jugador
    ctx.drawImage(iMedal, 760, 43, 32, 49);
    ctx.fillText(pad(level, score.toString().length), 815 + ((score.toString().length - 1) * 10), 68);
...
//                ctx.fillText('Presiona enter para reiniciar', canvas.width / 2, (canvas.height / 2) + 22);

//            } else {

//                ctx.fillText('Presiona el boton de disparo para reiniciar', canvas.width / 2, (canvas.height / 2) + 22);
...
function AddEnemies(totalEnemies) {
    ...
    for (var i = 0; i <= totalEnemies; i++) {
        ...
        enemies.push(new Rectangle(random(canvas.width / 10) * 10, random(30), 32, 32, GetTypeEnemy(), GetIncrement(), GetEnemiesHealth()));
    }
}
